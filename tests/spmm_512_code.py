...
def spmm(A, values, row_num, row_ptr, C):
    ...
    # A: M×K matrix = 512×512 = 262144 elements
    # C: M×N matrix = 512×512 = 262144 elements
    ...
    # Total memory footprint for size 512: 
    # ~2097680 bytes
    ...
    values = [1, 5, 3, 0, 2, 6, 4] * 8
    row_num = [1, 3, 2, 0, 1, 3, 2] * 8
    row_ptr = [i * 4 for i in [0, 2, 3, 6, 7]]

    # Block size for tiled computation
    BLOCK_SIZE = 64

    # Process computation in 64x64 blocks
    ...
    for i_block in range(0, M, BLOCK_SIZE):  # 8 iterations
        ...
        for j_block in range(0, N, BLOCK_SIZE):  # 8 iterations
            ...
            for i_local in range(BLOCK_SIZE):  # 64 iterations
                i = i_block + i_local
                if i >= M:
                    continue
                ...
                # pragma loop_count 64
                for j_local in range(BLOCK_SIZE):  # 64 iterations
                    j = j_block + j_local
                    if j >= N:
                        continue
                    ...
                    for k_ptr in range(k_start, k_end):  # Scales with 512
    ...
    print(f"Total expected computations: 3145728")
